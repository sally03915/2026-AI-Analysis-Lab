...
* **슬라이드 1제목:** LLM 환각 현상의 본질과 B2B 도입의 치명적 리스크
* 공공 인터넷 데이터 기반 사전학습의 한계로 인해 사내 고유 규정(연차, 경조사비 등)을 모르는 상태에서 그럴듯한 거짓말을 생성하는 매커니즘 해부.

* **슬라이드 2제목:** 프롬프트 펜싱(Prompt Fencing)과 Knowledge Grounding 아키텍처
* 프롬프트 최상단에 레퍼런스 문서를 주입하고, "문서 외 답변 절대 차단" 및 "모름 선언" 규칙을 강제하는 시스템 가드레일 설계.

> **🎤 [강사 멘트]**
> *"여러분, OpenAI 같은 최신 모델에게 '우리 회사 올해 경조사비 규정이 어떻게 되지?' 하고 물어보면 어떻게 될까요? 당연히 모릅니다. 모르면 모른다고 해야 하는데, AI는 특유의 '뻔뻔함'으로 가장 그럴듯한 거짓말을 지어냅니다. 이걸 전문 용어로 **환각(Hallucination)**이라고 하죠. 사내 인사팀 챗봇이 거짓말을 해서 직원이 잘못된 정보를 믿게 되면 큰일 나겠죠? 그래서 우리는 모델에게 '오직 내가 제공한 사내 규정 문서 안에서만 답하고, 모르면 모른다고 해라'라고 철저하게 목줄을 채우는 **Knowledge Grounding 기법**을 오늘 장착하게 될 겁니다."*


B2B AI 서비스 구축 시 가장 핵심이 되는 보안 및 신뢰성 확보 기술입니다. LLM이 학습한 궤적에 의존하지 않고, 강사가 주입한 콘텐트 내에서만 판단하도록 경계를 획정하는 아키텍처입니다.

* **B2B 관점의 기술적 의미:**
* **사전학습 데이터의 한계:** 일반적인 LLM은 공공 인터넷 데이터로 학습되어 사내 고유 규정(예: 연차 산정 기준, 경조사비 지급 표준)을 알 수 없음. 모르는 상태에서 강제로 답변을 요구하면 가장 그럴듯한 거짓말을 생성함.
* **Context Grounding (지식 결속):** 프롬프트 최상단에 참고 문서(Reference Document)를 주입하고, "반드시 제공된 문서 내용에만 기반하여 답변할 것. 문서에 없는 내용이라면 솔직하게 모른다고 대답할 것"이라는 강한 제약을 걸어 답변 범위를 가둡니다.
...
* **슬라이드 1제목:** Stateless(상태 비저장형) 인터프리터의 한계
* 사용자의 작은 인터랙션(버튼 클릭, 텍스트 입력) 하나에도 스크립트 전체가 처음부터 끝까지 리런(Re-run)되는 Streamlit 고유의 렌더링 아키텍처.
* **슬라이드 2제목:** `st.session_state`를 통한 메모리 격리 및 세션 관리
* UI가 갱신되어도 데이터가 증발하지 않도록 브라우저 탭 세션별 독립 힙 영역에 대화 이력을 캐싱하는 고가용성 설계 패턴.

> **🎤 [강사 멘트]**
> *"Streamlit은 파이썬 코드 몇 줄로 멋진 웹 UI를 뚝딱 만들어주지만, 여기에 엄청난 함정이 숨어 있습니다. 사용자가 채팅창에 글자 하나만 치고 엔터를 누르는 순간, 서버는 **소스코드의 1번 줄부터 마지막 줄까지 통째로 다시 실행**합니다. 일반 파이썬 변수에 대화 내역을 저장해 두면 리런될 때마다 초기화되면서 대화가 전부 증발해 버리죠. 이 마법 같은 프레임워크에서 대화가 끊기지 않게 유지해 주는 비기인 **`st.session_state`** 구조를 오늘 확실하게 내 것으로 만들어야 합니다."*

* **아키텍처 인사이트:**
* **상태 비저장형(Stateless) 인터프리터 아키텍처:** 사용자가 컴포넌트와 상호작용하면 소스 코드의 첫 줄부터 끝 줄까지 전체 스크립트를 재실행(Re-run)하므로 일반 파이썬 로컬 변수는 초기화됨.
* **`st.session_state` 시스템 캐시:** 서버 메모리 내 브라우저 탭 세션별 독립 공간을 제공하여 탑다운 재실행 시에도 데이터가 유지되도록 보장함.
