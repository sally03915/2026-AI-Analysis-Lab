...
딥러닝이 학습할 '재료'인 데이터를 컴퓨터가 어떻게 다루는지 맛보는 단계입니다. 사칙연산 수준의 코드로 시작합니다.[cite: 3]
*   **`basic001_numpy.ipynb`**: 수많은 숫자를 한 번에 계산해 주는 파이썬의 핵심 도구, Numpy와 친해지기[cite: 3]
...
*   **`basic006_미분의_구현.ipynb`**: 아주 작은 변화를 이용해 컴퓨터가 직접 미분을 계산하도록 코드로 구현해 보기[cite: 4]
*   **`basic004_딥러닝의_연쇄_오답노트_작성법.ipynb`**: 딥러닝의 꽃! 복잡한 계산을 한 번에 해결하는 '연쇄 오답노트(역전파)' 원리 맛보기

### 4단계. 실전 작동: 눈으로 보고 코드로 짜는 인공지능 (New)
수식은 최소화하고, 풍부한 그래프와 시각적 계산 흐름을 통해 인공지능이 실제로 데이터를 판별하고 학습하는 인프라를 완성합니다.[cite: 1, 2, 4]
*   **`basic007_신경망의_조립_퍼셉트론과_활성화함수.ipynb`**: 뉴런의 신호 전달 과정을 흉내 내고, 확률적 스위치(Sigmoid/ReLU)의 그래프 이해하기[cite: 1, 3]
*   **`basic008_오답의_무게_Cross_Entropy_손실함수.ipynb`**: 틀릴수록 벌점이 폭발하는 로그 그래프를 통해 모델의 채점 기준(손실함수) 배우기[cite: 2]
*   **`basic009_계산_그래프로_보는_역전파와_레이어_구현.ipynb`**: 사과/귤 영수증 계산 그래프 비유로 역전파의 진짜 코드(Affine, Softmax) 시각화하기[cite: 4]
*   **`basic010_밑바닥부터_조립하는_2층_신경망과_Keras_맛보기.ipynb`**: 레고 블록처럼 레이어를 조립해 전체 신경망을 완성하고, 고수준 API(Keras)의 편리함 체감하기[cite: 4]
...
2.  **공식을 외우지 마세요 (특히 4단계!)**
    *   4단계의 복잡한 행렬 수식이나 미분 기호에 겁먹을 필요 없습니다. 수식 대신 함께 첨부된 **'언덕 그래프', '로그 패널티 곡선', '화살표 계산 그래프'**의 움직임을 눈으로 이해하는 것이 훨씬 중요합니다.[cite: 1, 2, 4]
3.  **코드를 직접 실행해 보세요**
